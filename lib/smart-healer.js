...
class SmartHealer {
  ...
  async click(selector, options = {}) {
    ...
    try {
      if (useML) {
        // Use ML healer - get its internal healing time
        this._initMLHealer();
        const healingBefore = this.mlHealer.stats.totalHealingTimeMs;
        await this.mlHealer.click(selector, options);
        const healingAfter = this.mlHealer.stats.totalHealingTimeMs;
        this.stats.totalHealingTimeMs += (healingAfter - healingBefore);
        ...
        if (directSuccess) {
          // No healing occurred - don't track time
          console.log(`🟢 [HEALING-TIME-DEBUG] click() succeeded without healing: ${selector}`);
          this._recordSuccess(selector, false);
        } else {
          // Direct failed - get standaloneHealer's internal healing time only
          const healingBefore = this.standaloneHealer.stats.totalHealingTimeMs;
          console.log(`🔵 [HEALING-TIME-DEBUG] click() Before healing - standaloneHealer.stats.totalHealingTimeMs: ${healingBefore}ms`);
          
          await this.standaloneHealer.click(selector, options);
          
          const healingAfter = this.standaloneHealer.stats.totalHealingTimeMs;
          const healingDelta = healingAfter - healingBefore;
          console.log(`🔵 [HEALING-TIME-DEBUG] click() After healing - standaloneHealer.stats.totalHealingTimeMs: ${healingAfter}ms`);
          console.log(`🟡 [HEALING-TIME-DEBUG] click() Healing delta (search time only): ${healingDelta}ms`);
          
          this.stats.totalHealingTimeMs += healingDelta;
          console.log(`🟣 [HEALING-TIME-DEBUG] click() SmartHealer total healing time: ${this.stats.totalHealingTimeMs}ms`);
          this._recordSuccess(selector, false);
        }
      }
    } catch (error) {
  ...
  async fill(selector, value, options = {}) {
    ...
    try {
      if (useML) {
        ...
        if (directSuccess) {
          // No healing occurred - don't track time
          console.log(`🟢 [HEALING-TIME-DEBUG] fill() succeeded without healing: ${selector}`);
          this._recordSuccess(selector, false);
        } else {
          // Direct failed - get standaloneHealer's internal healing time only
          const healingBefore = this.standaloneHealer.stats.totalHealingTimeMs;
          console.log(`🔵 [HEALING-TIME-DEBUG] fill() Before healing - standaloneHealer.stats.totalHealingTimeMs: ${healingBefore}ms`);
          
          await this.standaloneHealer.fill(selector, value, options);
          
          const healingAfter = this.standaloneHealer.stats.totalHealingTimeMs;
          const healingDelta = healingAfter - healingBefore;
          console.log(`🔵 [HEALING-TIME-DEBUG] fill() After healing - standaloneHealer.stats.totalHealingTimeMs: ${healingAfter}ms`);
          console.log(`🟡 [HEALING-TIME-DEBUG] fill() Healing delta (search time only): ${healingDelta}ms`);
          
          this.stats.totalHealingTimeMs += healingDelta;
          console.log(`🟣 [HEALING-TIME-DEBUG] fill() SmartHealer total healing time: ${this.stats.totalHealingTimeMs}ms`);
          this._recordSuccess(selector, false);
        }
      }
    } catch (error) {
   ...
   * 🆕 FRAMEWORK FEATURE: Auto error detection on timeout
   */
  async waitFor(selector, options = {}) {
    ...
    try {
      if (useML) {
        ...
        if (directSuccess) {
          // No healing occurred - don't track time
          console.log(`🟢 [HEALING-TIME-DEBUG] waitFor() succeeded without healing: ${selector}`);
          this._recordSuccess(selector, false);
        } else {
          // Direct failed - get standaloneHealer's internal healing time only
          const healingBefore = this.standaloneHealer.stats.totalHealingTimeMs;
          console.log(`🔵 [HEALING-TIME-DEBUG] waitFor() Before healing - standaloneHealer.stats.totalHealingTimeMs: ${healingBefore}ms`);
          
          await this.standaloneHealer.waitFor(selector, options);
          
          const healingAfter = this.standaloneHealer.stats.totalHealingTimeMs;
          const healingDelta = healingAfter - healingBefore;
          console.log(`🔵 [HEALING-TIME-DEBUG] waitFor() After healing - standaloneHealer.stats.totalHealingTimeMs: ${healingAfter}ms`);
          console.log(`🟡 [HEALING-TIME-DEBUG] waitFor() Healing delta (search time only): ${healingDelta}ms`);
          
          this.stats.totalHealingTimeMs += healingDelta;
          console.log(`🟣 [HEALING-TIME-DEBUG] waitFor() SmartHealer total healing time: ${this.stats.totalHealingTimeMs}ms`);
          this._recordSuccess(selector, false);
        }
      }
    } catch (error) {
      ...
      // 🆕 FRAMEWORK FEATURE: Auto error detection on timeout
      // Check if timeout was due to an error message on page
      await this._enhanceErrorWithPageErrors(error, selector);
  ...
  /**
   * 🆕 FRAMEWORK FEATURE: Detect error messages on page
   * Automatically checks for common error selectors when waitFor() times out
   * @private
   */
  async _detectPageErrors() {
    // Check if auto error detection is enabled (default: true)
    if (this.options.autoErrorDetection === false) {
      return { found: false };
    }
    
    // Default error selectors (can be customized in config)
    const errorSelectors = this.options.errorSelectors || [
    ...
    // Try each error selector (fast check - 1s timeout per selector)
    for (const { selector, type } of errorSelectors) {
      try {
        ...
        if (isVisible) {
          // Found an error element - extract text
  ...
  /**
   * 🆕 FRAMEWORK FEATURE: Enhance timeout error with page error info
   * @private
   */
  async _enhanceErrorWithPageErrors(error, selector) {
    const errorInfo = await this._detectPageErrors();
    
    if (errorInfo.found) {
      // Add page error info to the error message
      const enhancedMessage = 
        `Timeout waiting for: ${selector}\n` +
        `🔴 Page Error Detected: ${errorInfo.message}\n` +
        `📋 Error Type: ${errorInfo.type}\n` +
        `🔍 Error Selector: ${errorInfo.selector}`;
      
      // Log to console for visibility
      console.error(`\n${'='.repeat(70)}`);
      console.error(`🔴 AUTO ERROR DETECTION (Framework Feature)`);
      console.error(`${'='.repeat(70)}`);
      console.error(`Expected Element: ${selector}`);
      console.error(`Page Error Found: ${errorInfo.type}`);
    ...
    return {
      click: async (selector, options = {}) => {
        ...
        try {
          ...
          console.log(`🟢 [HEALING-TIME-DEBUG] forFrame().click() trying original frame: ${frameSelector}`);
          await frameHealer.click(selector, options);
          console.log(`🟢 [HEALING-TIME-DEBUG] forFrame().click() succeeded without healing`);
          ...
          if (!isStringSelector) {
            ...
            try {
              // Track iframe healing time for performance metrics
              const healingStart = Date.now();
              console.log(`🔵 [HEALING-TIME-DEBUG] forFrame().click() Before iframe healing - parentStats.totalHealingTimeMs: ${parentStats.totalHealingTimeMs}ms`);
              
              const healedFrame = await parentHealer._healFailedFrame();
              
              const healingTime = Date.now() - healingStart;
              console.log(`🟡 [HEALING-TIME-DEBUG] forFrame().click() Iframe healing time: ${healingTime}ms`);
              
              parentStats.totalHealingTimeMs += healingTime;
              console.log(`🟣 [HEALING-TIME-DEBUG] forFrame().click() After iframe healing - parentStats.totalHealingTimeMs: ${parentStats.totalHealingTimeMs}ms`);
      ...
      fill: async (selector, value, options = {}) => {
        ...
        try {
          const frameHealer = parentHealer.standaloneHealer.forFrame(frameSelector);
          console.log(`🟢 [HEALING-TIME-DEBUG] forFrame().fill() trying original frame: ${frameSelector}`);
          await frameHealer.fill(selector, value, options);
          console.log(`🟢 [HEALING-TIME-DEBUG] forFrame().fill() succeeded without healing`);
          parentStats.standaloneSuccess++;
        } catch (error) {
          if (!isStringSelector) {
            console.log(`⚠️  Iframe interaction failed, attempting to heal iframe locator`);
            try {
              // Track iframe healing time for performance metrics
              const healingStart = Date.now();
              console.log(`🔵 [HEALING-TIME-DEBUG] forFrame().fill() Before iframe healing - parentStats.totalHealingTimeMs: ${parentStats.totalHealingTimeMs}ms`);
              
              const healedFrame = await parentHealer._healFailedFrame();
              
              const healingTime = Date.now() - healingStart;
              console.log(`🟡 [HEALING-TIME-DEBUG] forFrame().fill() Iframe healing time: ${healingTime}ms`);
              
              parentStats.totalHealingTimeMs += healingTime;
              console.log(`🟣 [HEALING-TIME-DEBUG] forFrame().fill() After iframe healing - parentStats.totalHealingTimeMs: ${parentStats.totalHealingTimeMs}ms`);
      ...
      waitFor: async (selector, options = {}) => {
        ...
        try {
          const frameHealer = parentHealer.standaloneHealer.forFrame(frameSelector);
          console.log(`🟢 [HEALING-TIME-DEBUG] forFrame().waitFor() trying original frame: ${frameSelector}`);
          await frameHealer.waitFor(selector, options);
          console.log(`🟢 [HEALING-TIME-DEBUG] forFrame().waitFor() succeeded without healing`);
          parentStats.standaloneSuccess++;
        } catch (error) {
          if (!isStringSelector) {
            console.log(`⚠️  Iframe interaction failed, attempting to heal iframe locator`);
            try {
              // Track iframe healing time for performance metrics
              const healingStart = Date.now();
              console.log(`🔵 [HEALING-TIME-DEBUG] forFrame().waitFor() Before iframe healing - parentStats.totalHealingTimeMs: ${parentStats.totalHealingTimeMs}ms`);
              
              const healedFrame = await parentHealer._healFailedFrame();
              
              const healingTime = Date.now() - healingStart;
              console.log(`🟡 [HEALING-TIME-DEBUG] forFrame().waitFor() Iframe healing time: ${healingTime}ms`);
              
              parentStats.totalHealingTimeMs += healingTime;
              console.log(`🟣 [HEALING-TIME-DEBUG] forFrame().waitFor() After iframe healing - parentStats.totalHealingTimeMs: ${parentStats.totalHealingTimeMs}ms`);

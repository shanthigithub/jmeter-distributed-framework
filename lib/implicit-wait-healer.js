...
class ImplicitWaitHealer {
  ...
  /**
   * WaitFor with implicit wait - adds polling to waitFor operations
   */
  async waitFor(selector, options = {}) {
    await this._waitForElement(selector, 'waitFor');
    // Still call healer.waitFor to ensure element meets visibility/state requirements
  ...
  forFrame(frameLocator) {
    ...
    return new ImplicitWaitHealer(frameHealer, {
...
async function createImplicitWaitHealer(page, config, healerInstance) {
  ...
  return new ImplicitWaitHealer(healerInstance, {
    implicitWaitMs,
    pollIntervalMs,
    logPolling
  });

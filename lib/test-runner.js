...
function appendToJTL(userId, actionName, durationMs, success, errorMsg = '') {
  ...
  const jtlLine = `${timestamp},${durationMs},${escapeCsvField(actionName)},${responseCode},${escapeCsvField(responseMessage)},${threadName},text,${success},${escapeCsvField(failureMessage)},0,0,1,1,,${durationMs},0,0\n`;
...
async function captureFailureScreenshot(userId, actionName, error) {
  try {
    ...
    // Capture screenshot - page is a direct property of browserContext

...
const { AICorrelationAnalyzer } = require('./ai-correlation-analyzer');
...
class SmartAPIGeneratorWithJMeter {
  constructor(options = {}) {
    ...
    this.anthropicBaseURL = options.anthropicBaseURL; // Optional: for TR proxy or custom endpoints
    this.aiCorrelations = [];
  }
  ...
  detectCorrelationsInRequest(call) {
    ...
    try {
      ...
      for (const [key, value] of this.correlationMap.entries()) {
        // Check for exact value match or URL-encoded match
        const valueStr = value.toString();
        const urlEncodedValue = encodeURIComponent(valueStr);
        
        if (bodyStr.includes(valueStr) || bodyStr.includes(urlEncodedValue)) {
          ...
          console.log(`🔗 Uses correlation: ${key} (found in request body)`);
        }
      }
      
      // Also check URL for correlations
      for (const [key, value] of this.correlationMap.entries()) {
        const valueStr = value.toString();
        if (call.url.includes(valueStr)) {
          call.usesCorrelation = call.usesCorrelation || [];
          call.usesCorrelation.push({
            variable: key,
            value: valueStr,
          ...
          console.log(`🔗 Uses correlation: ${key} (found in URL)`);
        }
      }
    } catch (e) {
...
${transaction.apiCalls.map(call => this.generateJMeterHTTPRequest(call)).join('\n')}
  ...
  generateJMeterHTTPRequest(call) {
    ...
    // Replace correlation values in body (handle both plain and URL-encoded)
    if (call.usesCorrelation) {
      call.usesCorrelation.forEach(corr => {
        // Replace plain value
        body = body.replace(new RegExp(this.escapeRegex(corr.value), 'g'), `\${${corr.variable}}`);
        
        // Replace URL-encoded value if it exists
        if (corr.urlEncodedValue && corr.urlEncodedValue !== corr.value) {
          body = body.replace(new RegExp(this.escapeRegex(corr.urlEncodedValue), 'g'), `\${${corr.variable}}`);
        }
      });
    }
...
${call.correlations ? call.correlations.map(corr => `            <JSONPostProcessor guiclass="JSONPostProcessorGui" testclass="JSONPostProcessor" testname="Extract ${corr.variable}">
  ...
  /**
   * Run AI analysis on captured API calls to detect correlations
   */
  async runAICorrelationAnalysis() {
    if (!this.useAI || !this.anthropicApiKey) {
      console.log(`\n⏭️  Skipping AI correlation analysis (useAI=${this.useAI}, API key=${!!this.anthropicApiKey})`);
      return;
    }

    console.log(`\n🤖 Running AI Correlation Analysis...`);
    
    // Pass baseURL option if configured (e.g., for Thomson Reuters proxy)
    const analyzerOptions = this.anthropicBaseURL ? { baseURL: this.anthropicBaseURL } : {};
    const analyzer = new AICorrelationAnalyzer(this.anthropicApiKey, analyzerOptions);
    
    // Add all captured API calls to analyzer
    for (const transaction of this.transactions) {
      for (const call of transaction.apiCalls) {
        analyzer.addAPICall(call);
      }
    }
    
    // Run AI analysis
    try {
      this.aiCorrelations = await analyzer.analyzeCorrelations();
      
      // Apply AI correlations to API calls
      this.applyAICorrelations(analyzer);
      
      console.log(`\n✅ AI Analysis Complete - Enhanced ${this.aiCorrelations.length} correlations`);
    } catch (error) {
      console.error(`\n⚠️  AI Analysis failed: ${error.message}`);
      console.log(`   Continuing with regex-based correlations...`);
    }
  }

  /**
   * Apply AI-detected correlations to API calls
   */
  applyAICorrelations(analyzer) {
    if (!this.aiCorrelations || this.aiCorrelations.length === 0) return;
    
    console.log(`\n🔧 Applying AI correlations to transactions...`);
    
    this.aiCorrelations.forEach(corr => {
      // Find the source call (where we extract the value)
      const sourceCall = this.findCallByNumber(corr.extractFromRequestNumber);
      if (!sourceCall) return;
      
      // Add AI correlation to source call
      if (!sourceCall.correlations) sourceCall.correlations = [];
      sourceCall.correlations.push({
        variable: corr.variableName,
        value: corr.valuePreview,
        type: 'ai_detected',
        extractPath: corr.jsonPath.replace('$.', ''),
        sourceUrl: sourceCall.url,
        sourceMethod: sourceCall.method,
        aiGenerated: true
      });
      
      // Mark calls that use this correlation
      corr.usedInRequests.forEach(reqNum => {
        const targetCall = this.findCallByNumber(reqNum);
        if (!targetCall) return;
        
        // Extract actual value from source response
        const actualValue = analyzer.extractValueByPath(sourceCall.responseBody, corr.jsonPath);
        if (!actualValue) return;
        
        // Add to usesCorrelation array
        if (!targetCall.usesCorrelation) targetCall.usesCorrelation = [];
        targetCall.usesCorrelation.push({
          variable: corr.variableName,
          value: actualValue.toString(),
          urlEncodedValue: encodeURIComponent(actualValue.toString()),
          location: 'body',
          aiGenerated: true
        });
        
        console.log(`   ✓ ${corr.variableName}: Request #${corr.extractFromRequestNumber} → Request #${reqNum}`);
      });
    });
  }

  /**
   * Find API call by sequence number (across all transactions)
   */
  findCallByNumber(sequenceNum) {
    let currentNum = 1;
    for (const transaction of this.transactions) {
      for (const call of transaction.apiCalls) {
        if (currentNum === sequenceNum) {
          return call;
        }
        currentNum++;
      }
  ...
  async saveGeneratedTests() {
    // Run AI analysis before generating tests
    await this.runAICorrelationAnalysis();
